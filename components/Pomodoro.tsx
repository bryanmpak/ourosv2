...
import { addTimeElapsed } from "../app/actions/pomodoro";
...
export default function Pomodoro({ dailyStreak }: PomodoroProps) {
  ...
  useEffect(() => {
    if (mode === "break" && timer === 0) {
      // server action to add time elapsed to DB
      addTimeElapsed(workTime);
      endCycle();
  ...
  return (
    <div className="h-full flex flex-col relative">
      <div className="text-center text-xl text-title mb-2 font-black absolute top-0 left-0 right-0">
      ...
      >
        <motion.div
          className="cursor-pointer w-[70%] sm:w-[85%]"
          onClick={() => handleClick()}
        ...
        <div className="w-1/3 h-[30px] mt-2 flex">
          <button
